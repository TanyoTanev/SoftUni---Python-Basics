...
Агенцията продава самолетни билети на различни авиокомпании. Ще получите информация за броя продадени билети за възрастни и
броя продадени детски билети. Нетната цена на билета за възрастен се определя от авиокомпанията, а детският билет е със 70% по-евтин.
...
avio_company = input()
tickets_adults_count = int(input())
tickets_kids_count = int(input())
tickets_adults_price = float(input())
service_price = float(input())

total_income = tickets_adults_count * tickets_adults_price + tickets_adults_price * 0.3 * tickets_kids_count + (tickets_kids_count + tickets_adults_count)* service_price

print(f"The profit of your agency from {avio_company} tickets is {total_income*0.2:.2f} lv.")